...
var _default = async (parent, args, context) => {
  ...
  if (args.type === 'add') {
    ...
    player[args.pouch === true ? 'pouchGold' : 'bankGold'] -= args.amount;
    if (args.pouch === true && player.pouchGold < 0) throw new Error("A player's pouch cannot go into a negative amount.");
    await player.save();
    await new _models.AdminLog({
      server: player.server,
      admin: context.user,
    ...
    const recipientPlayer = await _models.Player.findOne({
      server: args.serverID,
      guid: args.recipient
    });
    if (recipientPlayer === null) throw new Error('Could not find recipient player.');
